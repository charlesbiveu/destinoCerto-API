...
class Server {
 ...
 this.initilize(server)
  ...
  async middlewares(server){
    ...
    console.log('Checando middlewares concluídos... // Checking middlewares completed...')
  }


  async database(){
    try{
      console.log('Conectando ao banco de dados... // Connecting to database...')
      await connection.authenticate();
    } catch(error){
      console.log("Erro ao conectar ao banco // Error connecting to database",error)
    }
  }
  ...
  async initilize(server){
    ...
    server.listen(SERVER_PORT, () => {
      console.log(`Servidor iniciado na porta: ${SERVER_PORT}! // Starting server on port: ${SERVER_PORT}!`)
    })
  }
